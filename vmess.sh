...
getData() {
    ...
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        ...
        echo "   2) 小说站(随机选择)"
        echo "   3) 作者博客(http://liutaishuai.cn)"
        echo "   4) 网盘(https://pan.ikunn.world)"
        echo "   5) 自定义反代站点(需以http或者https开头)"
        read -p "  请选择伪装网站类型[默认:个人博客]" answer
        if [[ -z "$answer" ]]; then
            ...
            2)
                ...
                ;;
            3)
                PROXY_URL="http://liutaishuai.cn"
                ;;
            4)
                PROXY_URL="https://pan.ikunn.world"
                ;;
            5)
